...
echo "-----------------------------------------------------"
python -m adcircutils.mesh.mesh_merger input/channel.14 input/non-channel.14 -b vew -o output/ch_la.14 -d 'merged: channel + non-channel with VEWs' 

echo ""
echo "2. Combining channel and non-channel meshes by merging overlapping nodes"
echo "------------------------------------------------------------------------"
python -m adcircutils.mesh.mesh_merger input/channel.14 input/non-channel.14 -b merge -o output/ch_la_mgd.14 -d 'merged: channel + non-channel with merged nodes' 

echo ""
echo ""
echo "3. Subtracting channel + non-channel coverage from background"
echo "-------------------------------------------------------------"
python -m adcircutils.mesh.mesh_subtractor input/background.14 output/ch_la_mgd.14 -o output/background_subtracted.14 -d 'subtracted: background - (channel + non-channel)'

echo ""
echo ""
echo "4. Merging channel, non-channel, and subtracted background"
echo "-----------------------------------------------------------"
python -m adcircutils.mesh.mesh_merger output/ch_la.14 output/background_subtracted.14 -b merge -o output/ch_la_bg.14 -d 'merged: background_subtracted + channel + non-channel'

echo ""
echo ""
echo "5. Adding land boundaries to the merged mesh"
echo "--------------------------------------------"
python -m adcircutils.mesh.add_land_boundaries output/ch_la_bg.14 -o output/ch_la_bg_lb.14 -d 'merged: background_subtracted + channel + non-channel with land boundaries'
